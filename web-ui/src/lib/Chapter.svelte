<script>
  ...
  afterUpdate(async () => {
    /*
    console.log('Chapter.afterUpdate(): is_selecting[ %s : %s ], verse:',
                String( $is_selecting ),
                ($is_selecting ? $selected_store.join(', ') : 'null'),
                verse );
    // */

    // New target verse, if a verse number was requested, select and scroll.
    if (! $is_selecting) {
      remove_selection();
    }
    ...
    if (Array.isArray( verse_nums ) && verse_nums.length > 0) {
      ...
      if (verses.length > 0) {
        /*
        console.log('Chapter.afterUpdate(): verse_nums,verses:',
                    verse_nums, verses);
        // */
      ...
      if (first) {
        /*
        console.log('Chapter.afterUpdate(): scrollIntoView:', first);
        // */
